...
\begin{equation}
  ...
  \left\|
    ...
    \R^{K\times n}} \mathbf{w}
    \;+\;
    \underbrace{\big[p_\ell(\bx_j)\big]_{l,j}}_{=:P^T \in
    \R^{m\times n}} \mathbf{v}
    \;-\;
...
\eqref{eq:augment-constraint} penalized in the least squares objective. The motivation for this variant is robustness: anchoring
...
polar grid of $num\_rings$ concentric rings (spacing $h = r/num\_rings$, with $r$ the
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Dispatch between the direct system \eqref{eq:local-system} and
        ...
        variant with that many rings $num\_rings$.
\end{itemize}
...
assembled system (which tends to grow as stencils shrink or as $num\_rings$,
